...
Page({
  ...
  validateTap: function (e) {
    ...
    req.send({
      url: req.API.sendSmsCode + "?mobile=" + that.data.phone.value + "&type=rest_password"
    }).then(data => {
  ...
  btnTap: function (e) {
    const that = this;
    req.send({
      url: req.API.smsCodeValidate + "?mobile=" + that.data.phone.value + "&smsCode=" + that.data.pwd.value + "&type=rest_password"
    }).then(data => {
      if (data.data.success) {
        wx.navigateTo({
          url: "findPwdSubmit?phone=" + that.data.phone.value + "&smsCode=" + that.data.pwd.value,
        })
      } else {
        req.errorStatus(data.data.errCode);
      }
