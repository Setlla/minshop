...
function encryptByDES(desKey, checkCode) {
  const keyHex = CryptoJS.enc.Utf8.parse(desKey);
  const encrypted = CryptoJS.DES.encrypt(checkCode, keyHex, {
...
function getCheckCode(checkIndex, userId) {
  const charArr = checkIndex.split("");
  var checkCode = "";
